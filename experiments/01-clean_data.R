...
X_full <- data.frame(
      ...
      age = social$age, 
      treatment = as.numeric(social$treatment), 
...
X_unobserved <- data.frame(
  cluster = social$cluster,
  hh_id = social$cluster,
  hh_size = social$cluster)


save(X_full, X_unobserved, train_ids, test_ids, 
